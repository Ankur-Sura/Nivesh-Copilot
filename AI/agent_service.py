...
-----------------------------------
    User Query: "What's the stock price of Tata Motors?"
        ↓
    PLAN: "I need to get stock data. I should use the stock search tool."
        ↓
    ACTION: Call indian_stock_search("Tata Motors")
        ↓
    OBSERVE: "Stock price: ₹850..."
        ↓
    OUTPUT: "Tata Motors is trading at ₹850..."
...
3. get_current_datetime - Current date/time
...
def fallback_ddg(query: str, max_results: int = 3) -> List[Dict[str, str]]:
    ...
    try:
        ...
        resp = requests.get(
            ...
            headers={"User-Agent": "NiveshCopilot"},  # Identify ourselves
...
def run_agent(query: str, session_id: str = "default") -> Dict[str, Any]:
    ...
    Parameters:
    -----------
    query: User's question (e.g., "What's the stock price of Tata Motors?")
    ...
    SYSTEM_PROMPT = f"""
        ...
        Example Flow for Stock Query:
        User: "What's the stock price of Tata Motors?"
        Output: {{"step": "plan", "content": "User wants stock info. I'll use the stock search tool."}}
        Output: {{"step": "action", "function": "indian_stock_search", "input": {{"query": "Tata Motors"}}}}
        [You receive stock data]
        Output: {{"step": "output", "content": "Tata Motors is trading at ₹850..."}}
    """
    """
...
def agent_web_search(payload: Dict[str, str]):
    ...
    Body: { "query": "What's the latest stock news?", "session_id": "optional" }
...
# =============================================================================
#                     🆕 VOICE ORDER DETECTION ENDPOINT
# =============================================================================

@agent_router.post("/agent/detect-order")
def detect_order(payload: Dict[str, Any]):
    """
    📖 Voice Order Detection Endpoint
    ================================
    
    HTTP POST to: http://localhost:8000/agent/detect-order
    Body: { 
        "query": "Buy 10 shares of Tata Motors at 500"
    }
    
    Detects if user's query is an order command and extracts order details.
    Returns structured order information for confirmation.
    """
    query = payload.get("query")
    ...
    try:
        # Use LLM to detect and extract order details
        order_detection_prompt = f"""
        Analyze this user query and determine if it's a stock trading order command.
        
        Query: "{query}"
        
        If it's an order command, extract:
        1. Action: "BUY" or "SELL"
        2. Quantity: Number of shares
        3. Stock name: Company name
        4. Price: Limit price (if mentioned, otherwise "MARKET")
        ...
        {{
            ...
            "stock_name": "company name" or null,
            "price": number or "MARKET" or null,
            "confidence": 0.0-1.0
        }}
        
        Examples:
        - "Buy 10 shares of Tata Motors" → {{"is_order": true, "action": "BUY", "quantity": 10, "stock_name": "Tata Motors", "price": "MARKET"}}
        - "Sell 5 Reliance shares at 2500" → {{"is_order": true, "action": "SELL", "quantity": 5, "stock_name": "Reliance", "price": 2500}}
        - "What is the price of TCS?" → {{"is_order": false}}
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": order_detection_prompt}],
        ...
        return {
            "success": True,
            "is_order": order_info.get("is_order", False),
            "order_details": order_info if order_info.get("is_order") else None,
            "message": "Order detected" if order_info.get("is_order") else "Not an order command"
        }
        
    except Exception as err:
        print(f"❌ Order detection failed: {err}")
        raise HTTPException(status_code=500, detail=f"Order detection failed: {err}")
...
def run_smart_chat(
    ...
    force_tool: Manual override ("search", "stock", "news")
    ...
    if is_contextual and intent == "GENERAL":
        ...
        if thread_state and thread_state.get("messages"):
            ...
            if any(kw in history_text for kw in ["stock", "share", "nifty", "sensex", "share price", "market", "bse", "nse"]):
...
def smart_chat_endpoint(payload: Dict[str, Any]):
    ...
    Body: {
        ...
        "thread_id": "abc123",
        "user_id": "default",
        "force_tool": null  // Optional: "search", "stock", "news"
    }
...
4. 🆕 Multiple Tools: Web Search, Indian Stocks, News, DateTime
...
3. get_current_datetime- Current date and time
