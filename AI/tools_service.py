...
6. 🆕 DATE/TIME: Get current date and time
...
# ----- Third-Party Imports -----
import requests     # For HTTP requests (web search)
...
# =============================================================================
#                     🆕 YAHOO FINANCE STOCK PRICE TOOL
# =============================================================================

def get_stock_price_yahoo(symbol: str, exchange: str = "NSE") -> Dict[str, Any]:
    """
    📖 Yahoo Finance Stock Price - Get Real-Time Stock Data
    =======================================================
    
    Gets structured stock data from Yahoo Finance (unofficial API).
    This provides accurate prices, volume, market cap, and other metrics.
    
    Parameters:
    -----------
    symbol: Stock symbol (e.g., "TATAMOTORS", "RELIANCE", "TCS")
    exchange: "NSE" (default) or "BSE"
    
    Returns:
    --------
    Dict with: price, volume, market_cap, pe_ratio, etc.
    
    📌 INDIAN STOCK SYMBOLS:
    -----------------------
    Yahoo Finance uses:
    - NSE: "TATAMOTORS.NS" (add .NS suffix)
    - BSE: "TATAMOTORS.BO" (add .BO suffix)
    ...
    ✅ Fast response
    ✅ Accurate prices
    ✅ Additional metrics (P/E, volume, market cap)
    ✅ Historical data available
    
    ⚠️ LIMITATIONS:
    --------------
    - Unofficial API (may break or have rate limits)
    ...
    try:
        ...
        if exchange.upper() == "NSE":
            ...
            yahoo_symbol = f"{symbol.upper()}.NS"  # Default to NSE
        ...
        # If Yahoo Finance fails, return error (will fallback to web search)
        return {
            "error": f"Yahoo Finance lookup failed: {str(e)}",
            "symbol": symbol,
            "fallback": True
        }
...
def indian_stock_search(
    ...
    try_yahoo_first: bool = True  # 🆕 Try Yahoo Finance first for price queries
) -> Dict[str, Any]:
    """
    ...
    Now enhanced with Yahoo Finance for accurate price data!
    ...
    try_yahoo_first: If True, tries Yahoo Finance for price queries
    
    Returns:
    --------
    Dict with results from MoneyControl, Screener, ET, LiveMint
    + Yahoo Finance price data if available
    
    📌 ENHANCED WITH YAHOO FINANCE:
    -----------------------------
    🆕 If query looks like a stock price request:
    1. First tries Yahoo Finance for structured price data
    2. Falls back to web search if Yahoo Finance fails
    3. Combines both sources for comprehensive results
    
    📌 WHY THIS IS SPECIAL:
    ----------------------
    Normal search: Returns random blogs, outdated articles, spam
    This search: Returns ONLY from trusted financial sources
    + Structured price data from Yahoo Finance
    
    📌 HOW IT WORKS:
    ---------------
    1. Detects if query is asking for stock price
    2. Tries Yahoo Finance first (fast, structured data)
    3. Falls back to web search (MoneyControl, Screener, etc.)
    4. Combines results for best of both worlds
    ...
    5. 🆕 yahoo.com/finance - Structured price data
    
    🔗 FOR YOUR INTERVIEW:
    ----------------------
    "I enhanced the stock search with Yahoo Finance API to get
    structured price data, while maintaining web search fallback
    for comprehensive analysis from trusted Indian financial sites."
    """
    # 🆕 Try Yahoo Finance first for simple price queries
    yahoo_price_data = None
    if try_yahoo_first:
        # Detect if this is a simple stock price query
        price_keywords = ["price", "current price", "stock price", "share price", "trading at", "what is", "how much"]
        is_price_query = any(keyword in query.lower() for keyword in price_keywords)
        
        # Extract stock symbol from query (simple heuristic)
        # Look for common Indian stock names
        common_stocks = {
            "tata motors": "TATAMOTORS",
            "tata": "TATAMOTORS",  # Default to Tata Motors if just "Tata"
            "reliance": "RELIANCE",
            "hdfc": "HDFCBANK",
            "hdfc bank": "HDFCBANK",
            "infosys": "INFY",
        ...
        for stock_name, symbol in common_stocks.items():
            if stock_name in query_lower:
                # Try Yahoo Finance for structured price data
                try:
                    yahoo_result = get_stock_price_yahoo(symbol, "NSE")
                    if not yahoo_result.get("error") and yahoo_result.get("current_price"):
                        yahoo_price_data = yahoo_result
                        print(f"✅ Got structured price data from Yahoo Finance: {symbol} = ₹{yahoo_result.get('current_price')}")
                        break
                except Exception as e:
    ...
    # 🆕 Combine Yahoo Finance price data with web search results
    if yahoo_price_data:
        result["yahoo_price_data"] = yahoo_price_data
        result["has_structured_price"] = True
        # Add price summary to results
        price_summary = {
            "title": f"Current Price: {yahoo_price_data.get('company_name', 'Stock')}",
            "snippet": f"₹{yahoo_price_data.get('current_price')} | Volume: {yahoo_price_data.get('volume', 0):,} | P/E: {yahoo_price_data.get('pe_ratio', 'N/A')} | Market Cap: {yahoo_price_data.get('market_cap', 'N/A')}",
            "url": f"https://finance.yahoo.com/quote/{yahoo_price_data.get('yahoo_symbol')}",
            "source": "yahoo_finance"
        }
        # Insert price data at the beginning of results
        result["results"].insert(0, price_summary)
        print(f"✅ Combined Yahoo Finance price data with web search results")
    else:
        result["has_structured_price"] = False
...
async def transcribe_audio(
    ...
    try:
        ...
        # 🔧 FIX: Default to English to prevent Hindi transcription
        language_hint = (language or "").split("-")[0].lower() or "en"
        """
        📖 What is language_hint?
        -------------------------
        Language hint to help Whisper transcribe correctly.
        
        🔧 IMPORTANT: We default to "en" (English) because:
        - Indian users often speak English with Indian accent
        - Whisper can mistakenly detect as Hindi
        - Forcing English ensures accurate transcription
        
        If user explicitly passes language, we use that.
...
def search_web(query: str = Query(..., description="Search query text")):
    ...
    try:
        ...
        resp = requests.get(
            ...
            headers={"User-Agent": "Mozilla/5.0 NiveshCopilot"},
            timeout=10
        )
...
@tools_router.post("/tools/stock-prices")
async def stock_prices_endpoint(payload: Dict[str, Any]):
    """
    📖 Fetch Multiple Stock Prices (Yahoo Finance)
    ==============================================
    
    HTTP POST to: http://localhost:8000/tools/stock-prices
    Body: { "symbols": ["INFY", "TCS", "RELIANCE"] }
    
    Returns current prices for multiple stocks at once.
    Used by frontend for Watchlist and Holdings.
    """
    ...
    for symbol in symbols:
        try:
            data = get_stock_price_yahoo(symbol, "NSE")
            if not data.get("error"):
                results[symbol] = {
                    ...
                    "low_52w": data.get("52_week_low"),
                }
            else:
                results[symbol] = {"error": data.get("error")}
        except Exception as e:
...
@tools_router.get("/tools/market-indices")
async def market_indices_endpoint():
    """
    📖 Fetch Nifty 50 and Sensex Prices
    ===================================
    
    HTTP GET to: http://localhost:8000/tools/market-indices
    
    Returns current Nifty 50 and Sensex values.
    Used by frontend TopBar.
    """
    try:
        import yfinance as yf
        ...
        nifty_price = nifty_info.get("regularMarketPrice") or nifty_info.get("previousClose")
        nifty_change = nifty_info.get("regularMarketChangePercent", 0)
        
        # Fetch Sensex
        sensex = yf.Ticker("^BSESN")
        sensex_info = sensex.info
        sensex_price = sensex_info.get("regularMarketPrice") or sensex_info.get("previousClose")
        sensex_change = sensex_info.get("regularMarketChangePercent", 0)
        
        return {
            "nifty": {
                "price": round(nifty_price, 2) if nifty_price else None,
                "change": round(nifty_change, 2) if nifty_change else 0,
                "name": "NIFTY 50"
            },
            "sensex": {
                "price": round(sensex_price, 2) if sensex_price else None,
                "change": round(sensex_change, 2) if sensex_change else 0,
                "name": "SENSEX"
            },
            "fetched_at": datetime.utcnow().isoformat() + "Z"
        }
        ...
        return {
            "error": str(e),
            "nifty": {"price": None, "change": 0, "name": "NIFTY 50"},
            "sensex": {"price": None, "change": 0, "name": "SENSEX"}
        }


@tools_router.get("/tools/stock-price/{symbol}")
async def single_stock_price_endpoint(symbol: str):
    """
    📖 Fetch Single Stock Price (Yahoo Finance)
    ===========================================
    
    HTTP GET to: http://localhost:8000/tools/stock-price/TATAMOTORS
    
    Returns detailed price info for a single stock.
    Used by Buy/Sell popup to show current price.
    """
    # Name to Symbol mapping for common Indian stocks
    NAME_TO_SYMBOL = {
        "tata motors": "TATAMOTORS",
        "tata": "TATAMOTORS",
        "reliance": "RELIANCE",
    ...
    lookup_symbol = NAME_TO_SYMBOL.get(symbol.lower().strip(), symbol.upper().replace(" ", ""))
    
    try:
        data = get_stock_price_yahoo(lookup_symbol, "NSE")
...
6. GET  /tools/datetime        - Current date and time
